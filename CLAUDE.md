...
*Last updated: 2026-07-18 (**dashboard UX roadmap shipped** — Phase 0 corner/empty-state + Phase 2 Home "Today" glance [PR #30], Phase 3 month shelf + archive cards + Phase 4 capture heatmap [PR #31], and **Phase 1 optimistic add/edit/delete with background reconcile** [PR #32] — Apps Script redeployed for the client-UID echo, verified live on the owner's phone; see §3a + §Status. Prior, 2026-07-16: **one-handed ergonomics pass** — capture bar moved into a FAB-opened bottom sheet, FAB docked center of the nav pill, camera input also offers gallery, refresh icon removed; see §3a. Earlier: **Telegram bot DECOMMISSIONED** — the web app is now the whole product; `apps-script/Code.gs` is the single extraction/validation implementation. Earlier same day: repo decoupled from the bot; Phase 0 done — Firebase `project-alfred-f7575` configured [PRs #22/#23], capture bar, insights and the FCM push digest all VERIFIED LIVE on Android; nightly 10–11pm trigger set. Prior: independent-web-app build 2026-07-15.)*
...
doPost(e) routes add/edit/delete · handleAdd() (writes User col H; **honors a client-supplied `uid`, else `generateUID()` — returns `{success, uid}`**, redeployed 2026-07-18 for optimistic-write reconcile) · handleEdit() (writes User col H) · handleDelete() · backfillUIDs() · findRowByUID() · generateUID().
...
- **Optimistic writes (2026-07-18) — DONE & VERIFIED LIVE (PR #32).** Add / edit / delete / review-save now mutate `allRows` locally and re-render instantly (no full-screen loader, no blocking GViz round-trip), then POST in the background and fold in server truth via a **debounced reconcile** (`reconcileFromServer`, 1.5s after the POST). Reconcile keeps optimistic rows the GViz cache hasn't surfaced yet, honors optimistic deletes the cache still echoes, and de-dups on **UID first, content-signature second** — so it's correct whether or not the backend echoes the client UID. Failures roll the change back out of `allRows` and surface a neutral **toast** (`#toast`, inverse-surface, above the nav pill — not semantic red). Client sends a `clientUID()` with every add; `handleAdd` in `Code.gs` now honors a supplied `uid` (backward-compatible — older clients still get a server UID). **Apps Script redeployed 2026-07-18** so the UID-exact path is live; even without a redeploy the signature fallback covers it (so a redeploy is a precision upgrade, never required). Verified with the render loop (72 checks, 390/900 × light/dark: instant paint, reconcile confirm/dedup/lag, edit, delete, delete-under-lag, failure rollback + toast) **and confirmed live on the owner's phone** (instant add, no duplicate after reconcile, persists across reload, optimistic delete). See §3a.
