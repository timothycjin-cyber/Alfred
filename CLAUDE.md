...
- **Restructure Phase 3 — Today tab composition (2026-07-18) — DONE & VERIFIED.** Hero → tiles → glance line → shared live pace bar (`renderLivePaceBar`, per-block spring memory) → 14-day capture strip (heatmap's sienna ramp, `full grid in Trends` link); current-month-only rule for the live blocks; scroll-peek order verified at 390×700. Playwright: 23 checks incl. hand-computed glance values and ~60ms optimistic-add repaint. See §3a.

### What's Pending ❌
- **`ALFRED_RESTRUCTURE_ROADMAP_v2.md` Phases 4–7** (one per session, in order): Logs week index accordion, Trends month navigation/forecast/archive rework, full capture heatmap by `viewMonth`, optional FAB long-press
