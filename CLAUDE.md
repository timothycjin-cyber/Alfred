...
### What's Pending ❌
- **`ALFRED_REFINEMENT_ROADMAP_v3.md` Phases C–F** (one per session, in order): Today pace-card merge + Trends tile red-glow treatment (C), budget rename sweep (D), Subscriptions category (E), push digest retirement (F)
...
**The active roadmap is `ALFRED_REFINEMENT_ROADMAP_v3.md`** (mobile UI refinement + budget reframe; Phases A–B done, Phases C–F remain, one per session, in order). It supersedes the remaining `ALFRED_RESTRUCTURE_ROADMAP_v2.md` items: v2 Phase 6 heatmap polish moved to the v3 backlog, Phase 7 FAB long-press parked.
