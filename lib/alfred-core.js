...
(function (root, factory) {
  ...
  // ⚠️ IDEMPOTENCY IS PER SLOT, NOT PER DATE.
  //
  // recurringUID() keys an occurrence by the exact date it was written for. Move
  // a series' StartDate and every occurrence re-anchors onto a different date —
  // a different UID — so the "already written?" skip in materializeRecurring()
  // lets a SECOND row through for a period that is already filled. One monthly
  // income series put two rows into the same September that way.
  //
  // A series fills ONE SLOT PER CADENCE PERIOD: the month for monthly, the
  // Mon–Sun week for weekly, the day itself for daily. Keyed on the period the
  // guard survives an anchor move; keyed on the date it cannot.
  function occurrenceSlot(seriesId, cadence, iso) {
    if (cadence === 'monthly') return seriesId + '|' + iso.slice(0, 7);
    if (cadence === 'weekly') {
      var d = parseRowDate(iso);
      // Mon=0 … Sun=6. Unlike weekSpanFor() this does NOT clip to the month —
      // a week straddling a month boundary is still one week to a weekly series.
      var monday = new Date(d.getFullYear(), d.getMonth(), d.getDate() - ((d.getDay() + 6) % 7));
      return seriesId + '|w' + isoDateOf(monday);
    }
    return seriesId + '|' + iso;
  }

  // rc-<seriesId>-<YYYYMMDD> → { seriesId, iso }, or null for any other UID.
  // Series ids come from clientUID() (base36, no dashes), but the date is split
  // off the RIGHT regardless so an id carrying one cannot break the parse.
  function parseRecurringUID(uid) {
    var m = /^rc-(.+)-(\d{4})(\d{2})(\d{2})$/.exec(String(uid == null ? '' : uid));
    if (!m) return null;
    return { seriesId: m[1], iso: m[2] + '-' + m[3] + '-' + m[4] };
  }
  ...
  return {
    ...
    parseRecurringUID: parseRecurringUID,
